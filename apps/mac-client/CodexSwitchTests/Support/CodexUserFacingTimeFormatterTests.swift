...
final class CodexUserFacingTimeFormatterTests: XCTestCase {
    func testFormatterRendersDisplayLogAndFilenameTimestampsInRequestedTimeZone() {
        ...
        XCTAssertEqual(formatter.monthDayClockTimestamp(from: date), "03-28 18:31")
    ...
    func testFormatterUsesCurrentDefaultTimeZoneWhenNoOverrideIsProvided() {
        let originalTimeZone = NSTimeZone.default
        NSTimeZone.default = TimeZone(secondsFromGMT: 8 * 3600)!
        defer { NSTimeZone.default = originalTimeZone }

        let formatter = CodexUserFacingTimeFormatter()
        let date = Date(timeIntervalSince1970: 1_743_157_872)
